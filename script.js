const searchInput = document.getElementById('country-input'); const searchBtn = document.getElementById('search-btn'); const countryInfo = document.getElementById('country-info'); const bordersContainer = document.getElementById('borders'); const spinner = document.querySelector('.spinner');

async function searchCountry(countryName) { try { if (!countryName.trim()) { throw new Error('Please enter a country name.'); }
...
searchBtn.addEventListener('click', () => { const country = searchInput.value; searchCountry(country); });

searchInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') { searchCountry(searchInput.value); } });